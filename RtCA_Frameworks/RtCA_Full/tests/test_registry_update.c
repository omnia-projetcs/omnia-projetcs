                             ...
                             char *install_date_update,unsigned int session_id, sqlite3 *db)
{
  ...
  if (!CONSOL_ONLY || DEBUG_CMD_MODE)AddDebugMessage("test_registry_update", request, "-", MSG_INFO);
...
DWORD WINAPI Scan_registry_update(LPVOID lParam)
{
  ...
  AddDebugMessage("test_registry_update", "Scan registry updates  - START", "OK", MSG_INFO);
  ...
  if (hitem!=NULL) //files
  {
    while(hitem!=NULL)
    {
      ...
      if (file[0] != 0)
      {
        ...
        AddDebugMessage("test_registry_update", tmp_msg, "OK", MSG_INFO);
  ...
  AddDebugMessage("test_registry_update", "Scan registry updates  - DONE", "OK", MSG_INFO);
