...
%					 dans le cadre d'audits de s\'ecurit\'e, administration
...
\begin{description}
    \item[$\Square$] Sauvegarder l'\'etat des processus et liste des ports ouverts.
...
\begin{description}
    \item[$\Square$] Monter la partition.
    ...
    \item[$\Square$] Faire une recherche sur les types de fichier : scripts, sauvegardes, registre, multim\'edias, bureautiques, compress\'es, ex\'ecutables et temporaires\dots{}
    \item[$\Square$] Faire une recherche sur les derniers fichiers modifi\'es.
    \item[$\Square$] V\'erifier les droits des utilisateurs (administrateur\dots{}).
    \item[$\Square$] Recherche d'occurrences dans les fichiers de swap (cha\^ines de caract\'eres).    
\end{description} 

\vskip+0.2cm
\textbf{P\'eriph\'eriques :}
\begin{description}
    \item[$\Square$] Liste et date d'utilisation des p\'eriph\'eriques USB.
...
\begin{description}    
    ...
    \item[$\Square$] (Windows) Exportation des applications ex\'ecut\'ees, nombre d'utilisation et date de derni\'ere utilisation. 
\end{description}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\CficheTotal{}{Forensic - Outils (1/2)}
{
\vskip+0.1cm
\textbf{\large\color{red}Syst\`eme de fichiers et m\'emoire}\vskip+0.1cm
{\small
\begin{tabular}{ll}
\textbf{Nom/URL} & \textbf{Description} \\
\hline
\fl{\textbf{dd} (data dump)}{Permet d'effectuer des copies bit \`a bit de donn\'ees.\\\underline{Note :} En cas de disque endommag\'e ou besoin de reprise pr\'ef\'erer : {\sl\color{blue}ddrescue} packet gddrescue ou dc3dd \url{http://dc3dd.sourceforge.net}}{1.2cm}
...
%	\item  R\'ecup\'eration de fichiers effac\'es FAT/NTFS : \url{http://www.runtime.org/data-recovery-software.htm}
\end{tabular}
}\vskip+0.4cm
...
\fl{\textbf{VirusTotal}\\\url{http://www.virustotal.com}}{Analyse de fichier en ligne par un grand nombre d'antivirus.}{0.8cm}
\end{tabular}
}\vskip+0.4cm
...
\textbf{Nom/URL} & \textbf{Description} \\
\hline
\fl{\textbf{Evtx\_view}\\\url{http://www.tzworks.net}}{Extraction, analyse, export des hashs de mots de passe ...dans les fichiers de base de registre brut.}{0.8cm}
...
\fl{\textbf{LireExec}\\\url{http://omni.a.free.fr}}{Historique cach\'e de Windows des applications ex\'ecut\'ees par les utilisateurs. Extracteur inclus aussi dans {\sl\color{blue}CTRP}.}{0.8cm}
\end{tabular}
}\vskip+0.4cm
...
\textbf{Nom/URL} & \textbf{Description} \\
\hline
\fl{\textbf{Yaru}\\\url{http://www.tzworks.net}}{Extraction, analyse, export des hashs de mots de passe ...dans les fichiers de base de registre brut.}{0.8cm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\CficheTotal{}{Forensic - Outils (2/2)}
{
\vskip+0.1cm
...
\textbf{Nom/URL} & \textbf{Description} \\
\hline
\fl{\textbf{Nirsoft}\\\url{http://www.nirsoft.net}}{Liste d'outils permettant d'extraire les historiques de navigation, recherche et mots de passes.}{0.8cm}
...
\textbf{\large \color{red}Frameworks, suites d'outils et distributions}
{\small
\begin{itemize}
	...
	\item Suite d'outils pour l'analyse de fichiers journaux, registre, systme de fichier, application : \url{http://www.tzworks.net/download_links.php}
\end{itemize}
}\vskip+0.4cm

\textbf{\large \color{red}Ressources}
{\small
\begin{itemize}
	\item \url{http://forensiccontrol.com/fcresources.php}
	\item \url{http://www.forensicswiki.org}
	\item \url{http://www.forensic-computing.ltd.uk}
\end{itemize}
}

%%Ajouter : http://accessdata.com/support/adownloads et http://accessdata.com/downloads/current_releases/
...
\textbf{Sauvegarde de l'\'etat des services et ports : } {\sl \color{blue}netstat -lapute >etat\_proc.txt}\\
...
\begin{itemize}
	\item Droits des utilisateurs : {\sl \color{blue}/etc/sudoers, /etc/passwd, /etc/shadow, /etc/group, /etc/gshadow\dots{}}
	...
	%\item Scripts de d\'emarrage : {\sl \color{blue}/etc/rcX.d/}, 
\end{itemize}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vskip+0.1cm
Sous Linux un outil (en version alpha/unstable) existe :\\
 \url{http://www.nvlabs.in/archives/1-NVbit-Accessing-Bitlocker-volumes-from-linux.html}
...
\vskip+0.2cm
\textbf{Historique de navigation Internet :} (r\'ecup\'erer aussi les cookies)\\
IE : contenu dans le fichier {\sl \color{blue}index.dat}, lisible avec PASCO (Foundstone).\\
%Exemples d'emplacements : \url{C:/Documents and Settings/<user>/Local Settings/Temporary Internet Files:Content.IE5:}, \url{C:/Documents and Settings/<user>/Local Settings/History/History.IE5/},
%\url{C:/Documents and Settings/<user>/Cookies/}
Firefox/Chrome : avec {\sl \color{blue}f3e}, en copiant la base sqlite dans son r\'epertoire  (\url{http://www.firefoxforensics.com}).
...
\begin{description}
	\item[Babafou Latex] \url{http://tex.loria.fr/general/apprends-latex.pdf}
	\item[FAQ FR 2004] \url{http://omni.a.free.fr/Docs/faqfr-20041111-3.00.alpha.pdf}
	\item[Fancy Header] \url{http://www.cs.uu.nl/people/piet}
	\item[Symbols A4] \url{http://www.pakin.org/}
	\item[R\`egles de typographies] \url{http://jacques-andre.fr/faqtypo/lessons.pdf}
\end{description}
\vskip+0.3cm
...
{
%http://www.google.fr/url?sa=t&source=web&cd=3&ved=0CCwQFjAC&url=http%3A%2F%2Fgrub.enbug.org%2FAuthentification&rct=j&q=acc%C3%A8s%20a%20la%20console%20grub&ei=1SPyTKqhO8qwhAef7uCsDA&usg=AFQjCNGvG705vGvtCPCEdBO28ySGqjIpxQ&cad=rja
...
\vskip -0.5cm
 {
	...
	\begin{tabular}{cc}
		...
		{
			%Manque la configuration Wifi :
			%ifconfig wlan0 scan % pour identifier le rseau  
			% +se connecter et entre la cle
			% dhclient pour l'ip
			
			
			\MbFCmd{1.3cm}
			{Configuration du r\'eseau filaire :}
			{
			/etc/network/interface\\
			ifconfig
			}\\
			...
			{
			ifdown \textit{[interface]}\\
			ifup \textit{[interface]}
			}\\
			\MbFCmd{1.8cm}
...
\textbf{Trace route TCP avec sortie en 3D, PDF et SVG :}\vskip-0.1cm
{\sl \color{blue}
a = traceroute(["toto.fr","titi.fr","tutu.fr"])\vskip-0.1cm
a.trace3D()\vskip-0.1cm
a.pdfdump("export.pdf")\vskip-0.1cm
a.graph(target=">resultat.svg")}\vskip+0.1cm
...
\textbf{Mise en place :}
\begin{description}
   \item[$\Square$] Recherche la possibilit\'e d'inclure \`a une page notre propre code (PHP, javascript, lien) :
	\begin{itemize}
		\item livre d'or ;
		\item forum ;
		\item commentaire ;
...
%%Ajouter cette technique : http://www.nobodix.org/seb/win2003_adminpass.html
%%http://www.jms1.net/nt-unlock.shtml

%%http://fire.dmzs.com/index.php?section=tools&subsection=F
}
