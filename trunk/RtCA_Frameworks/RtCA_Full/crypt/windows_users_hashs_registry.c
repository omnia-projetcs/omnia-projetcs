...
//local function part !!!
//------------------------------------------------------------------------------
void addRegistryUsertoDB(char *name, char *RID, char *SID, char *group, char *description, char *last_logon, char *last_password_change,DWORD nb_connexion, char *type, DWORD state_id, DWORD session_id, sqlite3 *db);
    ...
    if (RegOpenKey(HKEY_LOCAL_MACHINE,"SAM\\SAM\\Domains\\Account\\Users\\",&CleTmp)==ERROR_SUCCESS)
    {
      ...
      if (RegQueryInfoKey (CleTmp,0,0,0,&nbSubKey,0,0,0,0,0,0,0)==ERROR_SUCCESS)
      {
        ...
        for (i=0;i<nbSubKey;i++)
        {
          ...
          {
            //if Windows < Vista
            if (os_type_XP) DecodeSAMHashXP(sk,User_infos.pwdump_pwd_raw_format,userRID,User_infos.name,b_f);
            else
            {
              /*a coder*/
