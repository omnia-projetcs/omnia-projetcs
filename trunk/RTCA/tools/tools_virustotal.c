...
DWORD WINAPI TCheckFileToVirusTotal(LPVOID lParam)
{
  ...
  CheckItemToVirusTotal(hlv, item, COLUMN_SHA256, COLUMN_VIRUSTOTAL,token);
...
{
  CheckItemToVirusTotal(hlstv, SendMessage(hlstv,LVM_GETNEXTITEM,-1,LVNI_FOCUSED), COLUMN_SHA256, COLUMN_VIRUSTOTAL, NULL);
  VIRUSTTAL = FALSE;
  return 0;
}
