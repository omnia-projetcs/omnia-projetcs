...
BOOL registry_users_extract()
{
  //import de la syskey
  char sk[MAX_PATH]="";
  registry_syskey(GetDlgItem(Tabl[TABL_REGISTRY],LV_REGISTRY_CONF),sk);
      ...
      if (RegQueryInfoKey (CleTmp,0,0,0,&nbSubKey,0,0,0,0,0,0,0)==ERROR_SUCCESS)
      {
        if (nbSubKey >0)
        {
          ...
          for (i=0;i<nbSubKey;i++)
          {
            ...
            {
              //génération du path utilisateur
              snprintf(path,MAX_LINE_SIZE,"SAM\\SAM\\Domains\\Account\\Users\\%s",NomSubKey);
              snprintf(lv_line[1].c,MAX_LINE_SIZE,"HKEY_LOCAL_MACHINE\\SAM\\SAM\\Domains\\Account\\Users\\%s",NomSubKey);

              lv_line[4].c[0]=0;
              size = LireGValeur(HKEY_LOCAL_MACHINE,path,"V",lv_line[4].c);
              if(size>0)
              {
        ...
        if (nbSubKey >0)
        {
          ...
          for (i=0;i<nbSubKey;i++)
          {
            ...
            if (RegEnumKeyEx (CleTmp,i,NomSubKey,&TailleNomSubKey,0,0,0,0)==ERROR_SUCCESS)
            {
              ...
              snprintf(lv_line[1].c,MAX_LINE_SIZE,"HKEY_LOCAL_MACHINE\\SAM\\SAM\\Domains\\Account\\Aliases\\%s",NomSubKey);
        ...
        if (nbSubKey >0)
        {
          ...
          for (i=0;i<nbSubKey;i++)
          {
            ...
            if (RegEnumKeyEx (CleTmp,i,NomSubKey,&TailleNomSubKey,0,0,0,0)==ERROR_SUCCESS)
            {
              ...
              snprintf(lv_line[1].c,MAX_LINE_SIZE,"HKEY_LOCAL_MACHINE\\SAM\\SAM\\Domains\\Builtin\\Aliases\\%s",NomSubKey);
