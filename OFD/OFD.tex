%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%En cours : 
%			- fiches SQL par type de BDD
%           - VI
%           - supprimer fiche Backtrack
%			- fiche DNS
%			- IRC + screen + issi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{l|l||l|l}
 ...
 % envoi a partir du storage, AT+CMSS=xxx (num du message envoy√© pr√©cedement)
 \multicolumn{4}{l}{}\\
...
{
\\Basic Input Output System (BIOS), permet la gestion des op\'erations \'el\'ementaires lors de sa mise sous tension. Il gËre la sÈquence de d\'emarrage et peut \^etre prot\'eg\'e par un mot de passe. Progressivement remplac\'e par l'Unified Extensible Firmware Interface (UEFI) qui permet beaucoup plus de param\'etrages.\\
Plusieurs outils permettent de passer outre le mot de passe de modification du BIOS. Ils exploitent la prÈsence d'un mot de passe constructeur ma\^itre permettant la r\'ecup\'eration du BIOS en cas de perte de mot de passe.
}
{
...
\begin{tabular}{|l|l|}
\hline
Fabricant & Mot de passe par dÈfaut\\\hline\hline
...
%Endocage : avec l'aide d'une cl\'e, les donn√©es sont transform\'ees, la cl\'e et le moyen d'encodage permettent de r\'ecup`'erer les donn\'ees initiales.
}
{
...
%chiffrement par secrets partag√©
...
{
	SystËme de publication de bureau et d'application \`a distance. Les serveurs sont en environnement Windows.
}
{
...
\begin{description}
    ...
    \item[$\Square$] Ne pas \'eteindre la machine, la d\'econnecter de tout r\'eseau en cas de risques de compromissions.
...
\textit{Une copie bit \`a bit d'un disque permet de copier tous les secteurs d'un disque, l'option \textbf{bs} permet d'optimiser la copie, ici pour une taille de secteur de 4096, peut \^etre \'etendu \`a la taille de buffer du disque (Pour identifier le disque {\sl \color{blue}fdisk -l}}).\\
...
\textbf{Syst\`eme :}\\
\textit{Politique de sÈcuritÈ local/de l'AD :}\\{\sl \color{blue}
...
\textbf{Remarques :}\\
L'impact du scanner sur une cible est le multiple du nombre de thread (ex\'ecution de test symultan\'ee) et de la criticitÈ des pluggins.\\
...
\textit{Du frameworks Origami codÈ en ruby, extraction d'objets d'un PDF (image, texte, scripts, polices) :}\\
...
\addcontentsline{toc}{subsubsection}{Samdump2}
\textit{Exclusivement pour les syst\`emes Windows XP/2003 et antÈrieurs.}\\
...
{
\\Secure Sockets Layer (SSL), est un protocole permettant le chiffrement des Èchanges rÈseau (couche transport).\\
...
\begin{description}
    ...
    \item[$\Square$] Les clÈs de chiffrement doivent Ítre au minimum de 128bits.
...
\textbf{SSLScan :} \textit{(Permet de v\'erifier les protocoles, suites cryptographiques et tailles de clÈ autoris\'ees.)\\
...
\begin{itemize}
	...
	\item Mettre \`a jour la copie locale : {\sl \color{blue}git checkout <URL>}
...
{
\\Les commandes suivantes sont utilisÈ afin de configurer directement en ligne de commande un serveur VMware.
}
{
...
\begin{itemize}
	\item {\sl \color{blue}mgmt-vmware} (Serveur hÙte)
	\item {\sl \color{blue}vmware-hostd} (Serveur hÙte)
...
\textbf{Ressources :}
\begin{itemize}
	\item RÈcapitulatif des id de journaux : \url{http://www.ultimatewindowssecurity.com/securitylog/quickref/Default.aspx}
\end{itemize}
