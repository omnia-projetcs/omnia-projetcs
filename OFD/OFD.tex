...
Ces informations sont divulgu\'ees \`a titre d'instruction, les auteurs et contributeurs ne peuvent \^etre rendus responsable de ce que vous en faites !}
\end{minipage}}
}
...
\begin{itemize}
	...
	\item T :  D\'emmare en mode disque cible, le disque dur est alors vue comme un support de stockage externe et peut être accéder via le port firewire (Target Mode)
...
\begin{itemize}
	\item Volume Header : header qui contient des donn\'ees \`a propos du syst\`eme de fichiers (date cr\'eation, taille des blocs etc.)
	\\ Il est situé 1024 octects après le début du volume et peut être analys\'e avec fsstat du Sleuthkit via une commande du type : {\sl \color{blue}fsstat -i raw -o OFFSETPARITION image.dd}
	\item Allocation file : fichier bitmap qui suit l'évolution de chaque bloc du volume
	\item Catalog file : fichier contenant les entr\'ees de chaque fichier et r\'epertoire (~~MFT)
\end{itemize}

\vskip+0.2cm
\textbf{Fichiers plist :}
Fichiers de configuration utilis\'es sur les systèmes Apple. 3 types existent : binaire, XML, JSON.\\
Possibilit\'e de convertir les plist binaire avec le script plutil.pl : \url{http://theiphonewiki.com/wiki/Plutil}

\vskip+0.2cm
\textbf{Fichiers Lowdown iOS (Escrow Keybag) :}
\\Le r\'epertoire /private/var/db/lockdown/ peut conteir des fichiers créés par le service Usbmuxd lors de la connection d'un iDevice déverouill\'e via USB autorisant l'ordinateur \`a communiquer avec le iDevice. Ce fichier permet de futurs connexions sans avoir \`a fournir le passcode même si le device est v\'erouill\'e ou si le passcode a chang\'e !

\vskip+0.2cm
\textbf{Fichiers et r\'epertoires importants :}
\begin{itemize}
	\item Date d'installation : {\sl \color{blue}/private/var/vm/swapfile} et {\sl \color{blue}/private/var/vm/sleepimage}
	\item Information sur le premier compte configur\'e : {\sl \color{blue}/private/var/db/.AppleSetupDone} 
	\item Configuration date et heure : {\sl \color{blue}/Library/Preferences/.GlobalPreferences.plist} 
	\item Logiciels install\'es via l'OS : {\sl \color{blue}/Library/Receipts}  
	...
	\item Fichiers d'hibernation et fichier d'\'echange : {\sl \color{blue}/private/var/vm/swapfile} et {\sl \color{blue}/private/var/vm/sleepimage}
	\item Version de l'OS : {\sl \color{blue}/System/Library/CoreServices/SystemVersion.plist}
	\item Configuration r\'eseau : {\sl \color{blue}/Library/Preferences/SystemConfiguration/preferences.plist} et {\sl \color{blue}/Library/Preferences/SystemConfiguration/com.apple.network.identification.plist}
	...
	\item Fichiers r\'ecents : {\sl \color{blue}/Users/username/Library/Preferences/com.apple.recentitems/plist}
